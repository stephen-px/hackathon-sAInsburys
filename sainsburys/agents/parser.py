...
def parse(user_slack_id: str, freeform: str, half: str) -> dict:
    ...
    or {rejected: reason} when the request isn't a food order.
    """
    user_msg = f"User: {user_slack_id}\nDelivery half: {half}\nRequest: {freeform}"
    resp = run_agent(SYSTEM, user_msg, TOOLS, IMPLS, max_turns=6, finisher=FINISHERS)
    ...
    raise RuntimeError(f"parser did not call a finisher tool: {resp}")
